describe("About Applying What We Have Learnt", () => {
  ...
  it("given I'm allergic to nuts and hate mushrooms, it should find a pizza I can eat (functional)", () => {
    ...
    productsICanEat =
      ...
      .filter((product) => !product.ingredients.some((ingredient) => ingredient === "mushrooms"))
  ...
  it("should add all the natural numbers below 1000 that are multiples of 3 or 5 (functional)", () => {
    ...
    /* try chaining Array.from() and reduce() */
    const sum =
      Array.from(Array(1000), (x = 0, i) => x + i)
      .reduce((sum, i) => {
        if (i % 3 === 0 || i % 5 === 0) return sum + i;
        return sum;
      })
  ...
  it("should count the ingredient occurrence (functional)", () => {
    ...
    ingredientCount =
      ...
      .reduce((sum, ingredients) => sum.concat(ingredients), [])
      .reduce((sum, ingredient) => {
        sum[ingredient] = (ingredient in sum ? sum[ingredient] + 1 : 1)
        return sum;
      }, {})
